...
/*   Updated: 2020/03/11 14:15:50 by frlindh          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			export(int ac, char **av, int out)
{
	...
	opex[1] = (ft_strcmp(av[0], "export")) ? 0 : 1;
	if ((i = -1) == -1 && opex[1] && i++ && ac == 1)
	...
	while (++i < ac)
	{
		...
		else if (av[i] && (j = -1) == -1)
		{
			...
			if ((opex[0] = *av[i]) != 0 && *av[i]++ == '+' && *av[i] != '=')
				out = bi_error(av[0], av[i], "not a valid identifier", 1);
			else if ((cpy[++j] = '\0') == '\0')
				(opex[0] == '+') ? set_var(cpy, opex[0], ++av[i], opex[1]) :
				set_var(cpy, opex[0], av[i], opex[1]);
		}
	}
	return (out);
}
