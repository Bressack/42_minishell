...
/*   Updated: 2020/02/15 16:05:31 by tharchen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	while (lexer__istype(lex->current_char, CHR_EOT))
		lexer__advance(lex);
}
...
t_token				lexer__get_word_token(t_lexer *lex)
{
	...
	printf("end of lexer__get_word_token\n");
	printf("%s\n", lex->line);
	printf("%*s\n",lex->pos + 1 , "^");
	return (new);
}
...
t_token				lexer__search_defined_token(t_lexer *lex)
{
	...
	while (g_defined_tokens[++i].type != NONE) // until the end of the array
	{
		if (g_defined_tokens[i].value == NULL)
			break ; // end of array
		if (g_defined_tokens[i].len < g_defined_tokens[max_match].len)
